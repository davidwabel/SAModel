...
def changeVarCost(df,tec_name,added_cost):
    df = df[df['technology']==tec_name]
    vtg = df[df['technology']==tec_name]['year_vtg']
    new_costs = df['value']+added_cost
    parameters = {'mode': 'M1', 
                  'node_loc': 'South Africa',
                  'technology': tec_name,
                  'time': 'year',
...
for ssp in ssp:
    ...
    changeVarCost(df,'bio_istig',5.86355616)
    changeVarCost(df,'coal_adv',30.51572307)
    changeVarCost(df,'coal_ppl',64.56503475)
    changeVarCost(df,'foil_ppl',22.51588277)
    changeVarCost(df,'loil_ppl',10.58553723)
    copy.solve('MESSAGE')
