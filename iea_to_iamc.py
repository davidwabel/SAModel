...
def append_Convert(df,out_Variable,PE_name,FEI_name,FENE_name,FERC_name,PEELEC_name,FETRP_name):
    a1 = df.loc[('History','IEA Energy Statistics (2017)','South Africa',PE_name,'EJ/yr')]
    a2 = df.loc[('History','IEA Energy Statistics (2017)','South Africa',FEI_name,'EJ/yr')]
    a3 = df.loc[('History','IEA Energy Statistics (2017)','South Africa',FENE_name,'EJ/yr')]
    a4 = df.loc[('History','IEA Energy Statistics (2017)','South Africa',FERC_name,'EJ/yr')]
    a5 = df.loc[('History','IEA Energy Statistics (2017)','South Africa',PEELEC_name,'EJ/yr')]
    a6 = df.loc[('History','IEA Energy Statistics (2017)','South Africa',FETRP_name,'EJ/yr')]
    
    df_test = (a1-a2-a3-a4-a5-a6)
    df_test['Unit'] = 'EJ/yr'
...
data = {
    ...
    ('total primary energy supply','total final consumption','elect.output in gwh','heat output in tj',):pe_map,
    ('industry sector','non-energy use industry/transformation/energy',):feind_map,
    ('transport sector',):fetrp_map,
    ('other sectors',):ferc_map,
    ('non-energy use',):fene_map,
    ('main activity producer electricity plants','main activity producer chp plants',
     'main activity producer heat plants',):pe_elec_map,
}
...
df.to_csv('IEA_mappedHistYears.csv', index=True)
